...
class TestIcosphere:
    ...
    def test_from_base_no_refinement(self):
        ...
        assert ico.num_nodes == 12
        assert ico.num_faces == 20
        assert ico.metadata.get('factor') == 1
    ...
    def test_from_base_with_refinement(self):
        ...
        assert ico.num_nodes > 12
        assert ico.num_faces > 20
        assert ico.metadata.get('factor') == refine_factor
    ...
    def test_from_base_rotation_preserves_unit_sphere(self):
        ...
        assert_array_almost_equal(unrotated_norms, np.ones(ico_unrotated.num_nodes))
    ...
    def test_from_base_consistent_results(self):
        ...
        ico1 = Icosphere.from_base(refine_factor=2, rotate=True, refine_by_angle=False)
        ico2 = Icosphere.from_base(refine_factor=2, rotate=True, refine_by_angle=False)
    ...
    def test_from_base_nodes_latlong_conversion(self):
        ...
        assert np.all(latlong[:, 0] >= -np.pi/2)
        assert np.all(latlong[:, 0] <= np.pi/2)
        
        # Longitude should be in [-π, π]
        assert np.all(latlong[:, 1] >= -np.pi)
        assert np.all(latlong[:, 1] <= np.pi)
