"""Profile and validate Cubesphere refinement: vectorized vs reference."""

import cProfile
import pstats
import time
import numpy as np
from sphedron import Cubesphere
from sphedron.refine import refine_rectrangles, _refine_rectrangles_reference

FACTORS = [16, 32, 64, 128]


def validate(factor=16):
    """Check vectorized matches reference for both use_angle modes."""
    nodes, faces = Cubesphere.base()
    from sphedron.transform import rotate_nodes
    nodes = rotate_nodes(nodes, axis="y", angles=Cubesphere.rotation_angles)

    for use_angle in [False, True]:
        n_vec, f_vec = refine_rectrangles(nodes, faces, factor, use_angle)
        n_ref, f_ref = _refine_rectrangles_reference(nodes, faces, factor, use_angle)
...
def benchmark():
    ...
    for factor in FACTORS:
        ...
        n_vec, f_vec = refine_rectrangles(nodes, faces, factor)
        dt_vec = time.perf_counter() - t0

        # Reference
        t0 = time.perf_counter()
        n_ref, f_ref = _refine_rectrangles_reference(nodes, faces, factor)
...
def profile_vectorized(factor=128):
    ...
    profiler.enable()
    refine_rectrangles(nodes, faces, factor)
    profiler.disable()
